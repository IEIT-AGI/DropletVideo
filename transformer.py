...
class DropletVideoPatchEmbed(nn.Module):
    def __init__(
            self,
            patch_size: int = 2,
            in_channels: int = 16,
...
class DropletVideoBlock(nn.Module):
    r"""
...
class DropletVideoTransformer3DModel(ModelMixin, ConfigMixin):
    """
    ...
    def __init__(
        ...
        # 1. Patch embedding
        self.patch_embed = DropletVideoPatchEmbed(patch_size, in_channels, inner_dim, text_embed_dim, bias=True)
        ...
        print("############################pos_embedding shape is {}########################".format(pos_embedding.shape))
        pos_embedding.data[:, max_text_seq_length:].copy_(spatial_pos_embedding)
        self.register_buffer("pos_embedding", pos_embedding, persistent=False)

        # 3. Time embeddings
        self.time_proj = Timesteps(inner_dim, flip_sin_to_cos, freq_shift)
        self.fps_proj = Timesteps(inner_dim, flip_sin_to_cos, freq_shift)
        self.time_embedding = TimestepEmbedding(inner_dim, time_embed_dim, timestep_activation_fn)
        self.fps_embedding = TimestepEmbedding(inner_dim, time_embed_dim, timestep_activation_fn)
    ...
    def forward(
        ...
        ###################################
        fps_emb = self.fps_proj(timestep_cond)
        ###########################################
        ...
        emb = self.time_embedding(t_emb, condition=None) + self.fps_embedding(fps_emb, condition=None)
        #############################################
        ...
        if not self.config.use_rotary_positional_embeddings:
            ...
            pos_embeds_without_text = pos_embeds[:, 226: ].view(1, self.post_time_compression_frames, self.post_patch_height, self.post_patch_width, emb_size)
